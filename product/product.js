...
const showFurniture = async(id) => {
    ...
    descriptionProduct.innerHTML = `
                ...
                // ne pas montrer option value="2" s'il n'y a rien
...
clickForCart.addEventListener('click', () => {
    if(typeof localStorage!='undefined'){ // si le localStorage est non défini alors crée un array vide
        ...
        /*
        let varnishStorage = [];
        let varnish = document.querySelector(".varnish");
        let choice = varnish.selectedIndex;
        let value = varnish.options[choice].value;
        */
        if(localStorage.getItem("listCart")) { // si localStorage a la clé listCart (il y a déjà des produits dans le panier) alors :
            ...
            //addVarnish(value, varnishStorage);
        ...
        /*
        function addVarnish(value, varnishStorage) {
            
            varnishStorage.push(value);
            localStorage.setItem("varnish", JSON.stringify(varnishStorage));
        }
        */
    }
